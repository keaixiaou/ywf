...
class Model {
    ...
    protected function _setFields(){
        if(empty($this->_fields)) {
            ...
            $data = ['sql' => $sql];
            $statement = $this->command($data);
            $statement->setFetchMode(\PDO::FETCH_ASSOC);
            $fieldsRes = $statement->fetchAll();
            if(!empty($fieldsRes)){
    ...
    public function startTrans(){
        $transId = uniqid();
        $data = $this->command(['trans_id'=>$transId, 'sql'=>'begin']);
    ...
    protected function endTrans($sql){
        ...
        $data = $this->command(['trans_id'=>$this->_transId,'sql'=>$sql]);
        return $data['result']===false?false:$data['result'];
    }
    ...
    protected function command($data){
        ...
        Db::setSql($sql);
        try {
            return $this->connect->executeSql($sql);
        }catch(\Exception $e){
